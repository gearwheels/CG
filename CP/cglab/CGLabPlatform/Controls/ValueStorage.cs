...
namespace CGLabPlatform{
    ...
    public class ValueStorage : UserControl{
            ...
            set{
                ...
                Debug.Assert(!(value is IBindable) || value.GetType().GetInterfaces().Intersect(IBindable)
                    .SingleOrDefault() != null, string.Format("Привязываемый к элементу управления ValueStorage " +
                                                              ...
                                                              "объекта при помощи оператора new.",
                    value.GetType().FullName.Replace('+', '.'), value.GetType().Name));
                _Storage = value;
        ...
        public Control GetControlForProperty(string name){
            var key = _PropControls.Keys.FirstOrDefault(k =>
                string.Compare(k.Name, name, false, CultureInfo.InvariantCulture) == 0);
        ...
        public ValueStorage(){
            ...
            SetVisibleScrollbars = (SetVisibleScrollbarsDelegate) typeof(ScrollableControl).GetMethod(
                    "SetVisibleScrollbars",
                    BindingFlags.NonPublic | BindingFlags.Instance)
                .CreateDelegate(typeof(SetVisibleScrollbarsDelegate), this);
        ...
        internal void UpdateLayout(){
            ...
            _Panel.Controls.Add(_Layout = new TableLayoutPanel()
                {Dock = DockStyle.Fill, ColumnCount = 2, Margin = new Padding(0), Padding = new Padding(4, 4, 8, 4)});
            ...
            foreach (var prop in _Storage.GetType().GetProperties(BindingFlags.Instance | BindingFlags.Public)){
                ...
                Debug.Assert(attr.SupportType(prop.PropertyType), string.Format(
                    "Невозможно осуществить связывание свойства {0} класса {1} " +
                    "с элементом управления, так как данное свойство помеченное аттрибутом {2} имеет не доступимый тип {3}.",
                    prop.Name,
                    (_Storage.GetType().GetInterfaces().Any(i => i == typeof(IBindable))
                        ? _Storage.GetType().BaseType
                        : _Storage.GetType())
                ...
                if (attr.Name != null){
                    _Layout.Controls.Add(_Control = new Label(){
                        Text = attr.Name, Font = Font, ForeColor = ForeColor, Dock = DockStyle.Fill,
                        AutoSize = true, Margin = new Padding(0, (_RowHeight - Font.Height) / 2, 0, 0)
                    }, 0, _Layout.RowCount);
                    var bind = new Binding("Font", bindings, "FontUpdate", false, DataSourceUpdateMode.Never);
...
//_Layout.RowCount++;
            ...
            for (var r = 0; r < _Layout.RowCount; ++r)
        ...
        public new Color ForeColor{
            get => bindings.ForeColor;
            set => bindings.ForeColor = value;
        }
        ...
        public new Font Font{
            ...
            set{
                bindings.Font = value;
                bindings.FontUpdate = !bindings.FontUpdate;
                ResizeLayoutInvalidate();
            }
        }
        ...
        private void SetDataSourceValue(PropertyInfo prop, object value = null){
            ...
            if (value.GetType() == prop.PropertyType)
                ...
                throw new Exception(string.Format("Невозможно инициализировать свойство {0} типа {1} " +
                                                  "значением {2} типа {3}", prop.Name, prop.PropertyType.Name, value,
                    value.GetType().Name));
        }
    ...
    public abstract class Bindable<T> : INotifyPropertyChanged, IBindable where T : Bindable<T>{
        ...
        public static T CreateInstance(){
            ...
            ILGenerator il;
            Debug.Assert(!typeofobj.IsSealed, string.Format(
                "Производный класс {0} унаследованный от Bindable<T> не может " +
                "являться запечатенным типом (объявленным с спецификатором sealed).",
                typeofobj.FullName.Replace('+', '.')));
            Debug.Assert(typeofobj.IsVisible, string.Format(
                "Производный класс {0} унаследованный от Bindable<T> должен " +
                "являться открытым типом или открытым вложенным типом.{1}Убедитесь, что класс {0} и все классы, " +
                "в которые он вложен, объявленны с спецификатором доступа public.",
                typeofobj.FullName.Replace('+', '.'), Environment.NewLine));
            ...
            var dintbuild = dmodbuild.DefineType("IBindable",
                TypeAttributes.Public | TypeAttributes.Abstract | TypeAttributes.Interface);
            ValueStorage.IBindable.Add(dintbuild.CreateType());
            var typebuild = dmodbuild.DefineType("Proxy_" + typeofobj.Name, TypeAttributes.Public |
                                                                            TypeAttributes.Sealed
                                                                            | TypeAttributes.BeforeFieldInit, typeofobj,
                new[]{ValueStorage.IBindable.Last()});

            var onPropertyChanged =
                typeofobj.GetMethod("OnPropertyChanged", BindingFlags.NonPublic | BindingFlags.Instance);
            var properties = typeofobj.GetProperties(BindingFlags.Public | BindingFlags.NonPublic |
                                                     BindingFlags.Instance | BindingFlags.DeclaredOnly);
            foreach (var property in properties.Where(p =>
                p.SetMethod != null && p.SetMethod.IsVirtual && !p.SetMethod.IsFinal
                ...
                Debug.Assert(property.SetMethod != null && property.GetMethod != null, string.Format("Свойство {0} " +
                    ...
                    "методов доступа получения и записи данных (get и set).", property.Name, typeofobj.FullName
                        .Replace('+', '.'), (property.SetMethod ?? property.GetMethod).IsAbstract
                        ? "абстрактным (abstract)"
                        : "виртулаьным (virtual)"));
                Debug.Assert(property.SetMethod.IsPublic && property.GetMethod.IsPublic, string.Format("Свойство {0} " +
                    ...
                    "или internal, а само свойство объявленно как public.", property.Name, typeofobj.FullName
                        .Replace('+', '.'), property.SetMethod.IsAbstract
                        ? "абстрактным (abstract)"
                        : "виртуальным " +
        ...
        protected P Get<P>([CallerMemberName] string name = null){
            ...
            if (_properties.TryGetValue(name, out value))
                return value == null ? default : (P) value;
            return default;
        }
    ...
    public abstract class DisplayPropertyAttribute : Attribute{
        ...
        protected internal readonly DisplayType displayType = DisplayType.Unknown;

        protected internal enum DisplayType{
            Unknown,
            Checker,
            Numeric,
            TextBox,
        ...
        protected internal DisplayPropertyAttribute(DisplayType DisplayType, string Property, object Default,
        ...
        public DisplayPropertyAttribute(object Default, string Property, string Name = null,
            DataSourceUpdateMode UpdateMode
                = DataSourceUpdateMode.OnPropertyChanged) : this(DisplayType.Unknown, Property, Default, Name,
            UpdateMode){ }

        protected Type ValueType => Default.GetType();

        public virtual double LineHeightFactor => 1d;

        public virtual bool SupportType(Type type){
            switch (Type.GetTypeCode(type)){
                case TypeCode.Int32:
                case TypeCode.UInt32:
                    if (displayType == DisplayType.ColorBox)
                        return true;
        ...
        internal Control CreateNewControl(){
    ...
    public sealed class DisplayCheckerPropertyAttribute : DisplayPropertyAttribute{
        ...
        public DisplayCheckerPropertyAttribute(bool Default, string Name)
            : base(DisplayType.Checker, "Checked", Default, null){
            this.Name = Name;
        }
    ...
    public sealed class DisplayNumericPropertyAttribute : DisplayPropertyAttribute{
        private decimal Minimum, Maximum, Increment;
        ...
        private DisplayNumericPropertyAttribute(object Default, string Name, dynamic Increment, dynamic Minimum,
            ...
            switch (displayType){
                case DisplayType.Numeric:
                    values = new[]{(decimal) this.Default};
                    break;
                case DisplayType.Vector2:
                    values = ((DVector2) this.Default).ToArray().Select(v => (decimal) v);
                    break;
                case DisplayType.Vector3:
                    values = ((DVector3) this.Default).ToArray().Select(v => (decimal) v);
                    break;
                case DisplayType.Vector4:
                    values = ((DVector4) this.Default).ToArray().Select(v => (decimal) v);
                    break;
                case DisplayType.Matrix3:
                    values = ((DMatrix3) this.Default).ToArray().Select(v => (decimal) v);
                    break;
                case DisplayType.Matrix4:
                    values = ((DMatrix4) this.Default).ToArray().Select(v => (decimal) v);
                    break;
            ...
            this.Decimals = Decimals >= 0
                ? Decimals
                : values.Concat(new[]{this.Increment}).Max(v =>
                    v.ToString().SkipWhile(c => c != ',' && c != '.').Skip(1).Count());
        }

        public override double LineHeightFactor{
            get{
                switch (displayType){
        ...
        private static object ConvertTo(object Default){
            ...
            if (type.IsArray && type.GetArrayRank() == 1){
                ...
                switch (Type.GetTypeCode(type.GetElementType())){
                    case TypeCode.Byte:
                        array = (Default as byte[]).Select(v => (double) v).ToArray();
                        break;
                    case TypeCode.UInt16:
                        array = (Default as ushort[]).Select(v => (double) v).ToArray();
                        break;
                    case TypeCode.UInt32:
                        array = (Default as uint[]).Select(v => (double) v).ToArray();
                        break;
                    case TypeCode.UInt64:
                        array = (Default as ulong[]).Select(v => (double) v).ToArray();
                        break;
                    case TypeCode.SByte:
                        array = (Default as sbyte[]).Select(v => (double) v).ToArray();
                        break;
                    case TypeCode.Int16:
                        array = (Default as short[]).Select(v => (double) v).ToArray();
                        break;
                    case TypeCode.Int32:
                        array = (Default as int[]).Select(v => (double) v).ToArray();
                        break;
                    case TypeCode.Int64:
                        array = (Default as long[]).Select(v => (double) v).ToArray();
                        break;
                    case TypeCode.Single:
                        array = (Default as float[]).Select(v => (double) v).ToArray();
                        break;
                    case TypeCode.Double:
                        array = Default as double[];
                        break;
                    case TypeCode.Decimal:
                        array = (Default as decimal[]).Select(v => (double) v).ToArray();
                        break;
                    default: return null;
                }
        ...
        protected override Control CreateControl(){
            switch (displayType){
                case DisplayType.Numeric:
                    return new NumericUpDownEx()
                        {Minimum = Minimum, Maximum = Maximum, Increment = Increment, DecimalPlaces = Decimals};
                case DisplayType.Vector2:
                    return new DVector2Edit()
                        {Minimum = Minimum, Maximum = Maximum, Increment = Increment, DecimalPlaces = Decimals};
                case DisplayType.Vector3:
                    return new DVector3Edit()
                        {Minimum = Minimum, Maximum = Maximum, Increment = Increment, DecimalPlaces = Decimals};
                case DisplayType.Vector4:
                    return new DVector4Edit()
                        {Minimum = Minimum, Maximum = Maximum, Increment = Increment, DecimalPlaces = Decimals};
                case DisplayType.Matrix3:
                    return new DMatrix3Edit()
                        {Minimum = Minimum, Maximum = Maximum, Increment = Increment, DecimalPlaces = Decimals};
                case DisplayType.Matrix4:
                    return new DMatrix4Edit()
                        {Minimum = Minimum, Maximum = Maximum, Increment = Increment, DecimalPlaces = Decimals};
                default: throw new NotSupportedException();
            }
        }

        public DisplayNumericPropertyAttribute(sbyte Default, sbyte Increment, string Name,
            sbyte Minimum = sbyte.MinValue, sbyte Maximum = sbyte.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum, 0){ }

        public DisplayNumericPropertyAttribute(short Default, short Increment, string Name,
            short Minimum = short.MinValue, short Maximum = short.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum, 0){ }

        public DisplayNumericPropertyAttribute(int Default, int Increment, string Name, int Minimum = int.MinValue,
            int Maximum = int.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum, 0){ }

        public DisplayNumericPropertyAttribute(long Default, long Increment, string Name, long Minimum = long.MinValue,
            long Maximum = long.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum, 0){ }

        public DisplayNumericPropertyAttribute(byte Default, byte Increment, string Name, byte Minimum = byte.MinValue,
            byte Maximum = byte.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum, 0){ }

        public DisplayNumericPropertyAttribute(ushort Default, ushort Increment, string Name,
            ushort Minimum = ushort.MinValue, ushort Maximum = ushort.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum, 0){ }

        public DisplayNumericPropertyAttribute(ulong Default, ulong Increment, string Name,
            ulong Minimum = ulong.MinValue, ulong Maximum = ulong.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum, 0){ }

        public DisplayNumericPropertyAttribute(float Default, float Increment, string Name,
            float Minimum = (float) long.MinValue, float Maximum = (float) long.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum){ }

        public DisplayNumericPropertyAttribute(float Default, float Increment, int Decimals, string Name,
            float Minimum = (float) long.MinValue, float Maximum = (float) long.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum, Decimals){ }

        public DisplayNumericPropertyAttribute(double Default, double Increment, string Name,
            double Minimum = (double) long.MinValue, double Maximum = (double) long.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum){ }

        public DisplayNumericPropertyAttribute(double Default, double Increment, int Decimals, string Name,
            double Minimum = (double) long.MinValue, double Maximum = (double) long.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum, Decimals){ }

        public DisplayNumericPropertyAttribute(decimal Default, decimal Increment, string Name,
            decimal Minimum = decimal.MinValue, decimal Maximum = decimal.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum){ }

        public DisplayNumericPropertyAttribute(decimal Default, decimal Increment, int Decimals, string Name,
            decimal Minimum = decimal.MinValue, decimal Maximum = decimal.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum, Decimals){ }
        ...
        public DisplayNumericPropertyAttribute(DVector2 Default, double Increment, string Name,
            double Minimum = (double) long.MinValue, double Maximum = (double) long.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum){ }

        public DisplayNumericPropertyAttribute(DVector2 Default, double Increment, int Decimals, string Name,
            double Minimum = (double) long.MinValue, double Maximum = (double) long.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum, Decimals){ }

        public DisplayNumericPropertyAttribute(DVector3 Default, double Increment, string Name,
            double Minimum = (double) long.MinValue, double Maximum = (double) long.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum){ }

        public DisplayNumericPropertyAttribute(DVector3 Default, double Increment, int Decimals, string Name,
            double Minimum = (double) long.MinValue, double Maximum = (double) long.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum, Decimals){ }

        public DisplayNumericPropertyAttribute(DVector4 Default, double Increment, string Name,
            double Minimum = (double) long.MinValue, double Maximum = (double) long.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum){ }

        public DisplayNumericPropertyAttribute(DVector4 Default, double Increment, int Decimals, string Name,
            double Minimum = (double) long.MinValue, double Maximum = (double) long.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum, Decimals){ }

        public DisplayNumericPropertyAttribute(DMatrix3 Default, double Increment, string Name,
            double Minimum = (double) long.MinValue, double Maximum = (double) long.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum){ }

        public DisplayNumericPropertyAttribute(DMatrix3 Default, double Increment, int Decimals, string Name,
            double Minimum = (double) long.MinValue, double Maximum = (double) long.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum, Decimals){ }

        public DisplayNumericPropertyAttribute(DMatrix4 Default, double Increment, string Name,
            double Minimum = (double) long.MinValue, double Maximum = (double) long.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum){ }

        public DisplayNumericPropertyAttribute(DMatrix4 Default, double Increment, int Decimals, string Name,
            double Minimum = (double) long.MinValue, double Maximum = (double) long.MaxValue)
            : this(Default, Name, Increment, Minimum, Maximum, Decimals){ }
    }
    ...
    public sealed class DisplayColorBoxPropertyAttribute : DisplayPropertyAttribute{
        ...
        public DisplayColorBoxPropertyAttribute(int Default, string Name)
            : base(DisplayType.ColorBox, "Value", Default, null){
            this.Name = Name;
        }
        ...
        public DisplayColorBoxPropertyAttribute(DVector3 Default, string Name)
            : this(
                0xFF | ((int) (255 * Default.X) & (0xFF << 24)) | ((int) (255 * Default.Y) & (0xFF << 16)) |
                ((int) (255 * Default.Z) & 0xFF), Name){ }
