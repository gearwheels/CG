...
namespace CGLabPlatform{
    public class GDIDeviceUpdateArgs : IDeviceUpdateArgs{
        ...
        /// </summary>
        public double Width => Surface.Width;

        /// <summary>
        /// Высота буфера в пикселях элемента управления GDIDevice
        /// </summary>
        public double Heigh => Surface.Height;
    ...
    public class GDIDevice : DrawDevice<GDIDeviceUpdateArgs>{
        ...
        public bool GraphicsHighSpeed{
            get => _dibdmode == SurfaceDrawQuality.Low;
            set{
        ...
        public BitmapSurface DIB => _DIB;
        public GDIGraphics GFX => _GFX;
        public bool Disposed => _Disposed || DIB.Disposed;
        ...
        [DllImport("gdi32")]
        private static extern unsafe int SetDIBitsToDevice(HandleRef hDC, int xDest, int yDest, int dwWidth,
            int dwHeight,
    ...
    public unsafe class BitmapSurface : IDisposable{
        ...
        private static extern IntPtr CreateFileMapping(IntPtr hFile, IntPtr lpFileMappingAttributes, uint flProtect,
            uint dwMaximumSizeHigh, int dwMaximumSizeLow, string lpName);

        [DllImport("kernel32.dll", SetLastError = true)]
        private static extern IntPtr MapViewOfFile(IntPtr hFileMappingObject, uint dwDesiredAccess,
            uint dwFileOffsetHigh, uint dwFileOffsetLow, int dwNumberOfBytesToMap);
        ...
        public int this[int x, int y]{
            get => *(int*) (ImageBits + y * stride + (x << 2));
