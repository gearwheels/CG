...
namespace CGLabPlatform{
    ...
    public class OGLDevice : DrawDevice<OGLDeviceUpdateArgs>{
        ...
        public int VSync{
            get{
                return glContext.Send(() => gl.IsExtensionFunctionSupported
                    ...
                    ? gl.GetSwapIntervalEXT()
                    : -1);
            }
            set{
                glContext.Post(() => {
                    if (gl.IsExtensionFunctionSupported
        ...
        public bool Multisample{
            ...
            get{
                return glContext.Send(() => {
                    byte[] r1 = new byte[1], r2 = new byte[1]{0};
        ...
        public OGLDevice(){
            ...
            uiContext = SynchronizationContext
                .Current; // NOTE: Сделанно допущение, что объект всегда создается из потока UI   
            HandleDestroyed += (sender, args) => {
                if (Closed != null){
                    lock (LockObj){
                        DeviceUpdate = null; // Нет смысла вызывать обновление устройства после обратки события
                    }
        ...
        private void RenderLoop(){
            ...
            var fps_count = 0;
            var fps_ticks = 0;
            var pre_ticks = Environment.TickCount & int.MaxValue;
            ...
            gl.BlendFunc(OpenGL.GL_SRC_ALPHA,
                OpenGL.GL_ONE_MINUS_SRC_ALPHA); // result_color = src * src.a + dst * (1 - src.a)
            ...
            while (true){
                ...
                var cur_ticks = Environment.TickCount & int.MaxValue;
                ...
                lock (LockObj){
                    if (DeviceUpdate != null){
                        ...
                        gl.Flush();
                        OpenGL.Blit(
                            handle); // Для ContextType.NativeWindow тут выполняется переключение буфферов,                                                 
    ...
    public class OGLDeviceUpdateArgs : OpenGLEventArgs, IDeviceUpdateArgs{
        public double Width => gl.RenderContextProvider.Width;
        public double Heigh => gl.RenderContextProvider.Height;
