...
namespace SharpGL
{
    public partial class OpenGL
    {
        ...
        {
            IntPtr p = Marshal.AllocHGlobal(data.Length * sizeof(double));
            Marshal.Copy(data, 0, p, data.Length);
        ...
        {
            IntPtr p = Marshal.AllocHGlobal(pointer.Length * sizeof(float));
            Marshal.Copy(pointer, 0, p, pointer.Length);
            GetDelegateFor<glVertexAttribPointer>()(index, size, OpenGL.GL_FLOAT, normalized, stride, p);
            Marshal.FreeHGlobal(p);
        }
        public unsafe void VertexAttribPointer(uint index, int size, bool normalized, int stride, float* pointer)
        {
            GetDelegateFor<glVertexAttribPointer>()(index, size, OpenGL.GL_FLOAT, normalized, stride, (IntPtr)pointer);
        }
        public void VertexAttribPointer(uint index, int size, bool normalized, int stride, double[] pointer)
        {
            IntPtr p = Marshal.AllocHGlobal(pointer.Length * sizeof(double));
            Marshal.Copy(pointer, 0, p, pointer.Length);
            GetDelegateFor<glVertexAttribPointer>()(index, size, OpenGL.GL_DOUBLE, normalized, stride, p);
            Marshal.FreeHGlobal(p);
        }
        public unsafe void VertexAttribPointer(uint index, int size, bool normalized, int stride, double* pointer)
        {
            GetDelegateFor<glVertexAttribPointer>()(index, size, OpenGL.GL_DOUBLE, normalized, stride, (IntPtr)pointer);
        }
